...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		valstr = row2vals(row)
		cmd = f"INSERT INTO TEMP_{TableName} VALUES ({valstr});"
		cmdlist.append(cmd)
	return cmdlist
...
""")
                print(f"Created TEMP_{TableName}")
...
def load(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		...
		print("Insertion into temporary table complete")
		cursor.execute(f"INSERT INTO {TableName} SELECT * FROM TEMP_{TableName}")
