...
def getRowValues(rowlist):
        valueList = []
        for row in rowlist:
            valueList.append(row2vals(row))
        return valueList
...
def loadBatch(conn, values):
        psycopg2.extras.execute_batch(conn.cursor(), f'INSERT INTO {TableName} VALUES {values}', values)
        conn.commit()

def copyData(conn, fname):
        ...
        with open(fname, mode='r') as f:
            with conn.cursor() as cursor:
                start = time.perf_counter()
                cursor.copy_expert(f'COPY {TableName}(CensusTract, State, County, TotalPop, Men, Women, Hispanic, White, Black, Native, Asian, Pacific, Citizen, Income, IncomeErr, IncomePerCap, IncomePerCapErr, Poverty, ChildPoverty, Professional, Service, Office, Construction, Production, Drive, Carpool, Transit, Walk, OtherTransp, WorkAtHome, MeanCommute, Employed, PrivateWork, PublicWork, SelfEmployed, FamilyWork, Unemployment) FROM STDIN WITH HEADER CSV', f)
                elapsed = time.perf_counter() - start
                print(f'Finished Loading. Elapsed Time: {elapsed:0.4} seconds')
...
def main():
    ...
    values = getRowValues(rlis)

    if CreateDB:
    	createTable(conn)

    #load(conn, cmdlist)
    copyData(conn, Datafile)
